...
mod test {
    ...
    fn test_parse_bin() {
        ...
        let items = vec![
            Message {
                data: Bytes::from_static(GET_REQ_BIN_DATA),
                flags: CmdFlags::empty(),
                mtype: MsgType::Binary {
                    btype: BinType::Req,
                    bmtype: BinMsgType::GetK,
            ...
            // next is for binary cases
            Message {
                data: Bytes::from_static(GET_RESP_BIN_DATA),
                flags: CmdFlags::empty(),
                mtype: MsgType::Binary {
                    btype: BinType::Resp,
                    bmtype: BinMsgType::GetK,
            ...
            MsgType::Binary { bmtype, .. } => match bmtype {
                BinMsgType::GetKQ | BinMsgType::GetQ => {
                    ...
                    let status = match cursor.read_u16::<BigEndian>() {
                        Ok(status) => status,
                        Err(err) => {
                            tracing::warn!("fail to parse status code {}", err);
